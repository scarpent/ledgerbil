...
@pytest.mark.parametrize(
    "test_input",
    ["; comment", "  ; indented comment", "    ; indented comment", "a: something"],
)
...
class ReconcilerParsing(Redirector):
    ...
    def test_simple_transactions(self):
        self.verify_reconcile_vars(
            ["2016/10/23 blah", "    e: blurg      $25", "    a: checking   $-25"],
            account="check",
    ...
    def test_simple_transactions_with_math(self):
        self.verify_reconcile_vars(
            ["2016/10/23 blah", "    e: blurg      $25", "    a: checking"],
            account="check",
            expected_match="a: checking",
            expected_amount=-25,
        )
        ...
        self.verify_reconcile_vars(
            ["2016/10/23 blah", "    a: checking", "    e: blurg      $25"],
            account="check",
            expected_match="a: checking",
            expected_amount=-25,
        )
    ...
    def test_comma_in_amount(self):
        self.verify_reconcile_vars(
            ["2016/10/23 blah", "    e: blurg      $1000", "    a: checking   $-1,000"],
            account="check",
            expected_match="a: checking",
...
def test_mixed_shares_and_non_shares_raises_exception():
    ...
    lines = ["2018/01/08 blah", "    a: xyz", "    a: xyz  1.234 abc @ $10"]
    with pytest.raises(LdgReconcilerError) as excinfo:
...
def test_mixed_shares_and_non_shares_raises_exception_too():
    ...
    lines = ["2018/01/08 blah", "    a: xyz  1.234 abc @ $10", "    a: xyz"]
    with pytest.raises(LdgReconcilerError) as excinfo:
        LedgerThing(lines, reconcile_account="xyz")
    formatted_lines = "\n".join(lines)
    expected = f"Unhandled shares with non-shares:\n{formatted_lines}"
...
def test_shares():
    lines = ["2018/01/08 blah", "    a: xyz  1.234 abc @ $10", "    a: abc"]
...
def test_lines_is_different_than_get_lines_when_status_changes():
    lines = ["2018/01/08 blah", "    e: xyz", "    l: abc         $-10"]
    lines_with_status = ["2018/01/08 blah", "    e: xyz", "  ! l: abc         $-10"]
