...
from .. import settings, settings_getter, util


class MockSettings:
    DATE_FORMAT = settings_getter.defaults["DATE_FORMAT"]
...
@pytest.mark.parametrize(
    "test_input, expected",
    [("2016/10/26", True), ("2016/1/5", True), ("2016/5/5 10:23", False)],
)
def test_is_valid_date(test_input, expected):
    assert util.is_valid_date(test_input) is expected
...
def test_is_integer(test_input, expected):
    assert util.is_integer(test_input) is expected
...
def test_is_float(test_input, expected):
    assert util.is_float(test_input) is expected
...
@pytest.mark.parametrize(
    "test_input, expected", [([1, 7, 2], 14), ({9, 7, 3}, 189), ([], 1)]
)
...
@pytest.mark.parametrize(
    "test_input, expected", [("", []), (None, []), ('a b "c d"', ["a", "b", "c d"])]
)
...
@pytest.mark.parametrize(
    "test_input, expected",
    [('"open', None), ('"still open', None), ("this also counts as open'", None)],
)
