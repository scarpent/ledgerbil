...
def test_bimonthly():
    ...
    run_it(testdate, testdate + relativedelta(months=2), "bimonthly")
...
def test_quarterly():
    ...
    run_it(testdate, testdate + relativedelta(months=3), "quarterly")
...
def test_biannual():
    ...
    run_it(testdate, testdate + relativedelta(months=6), "biannual")


def test_run_enter_days_less_than_one():
    schedulefiledata = FT.read_file(FT.test_enter_lessthan1)
    with FT.temp_file(schedulefiledata) as temp_schedule_filename:
        with FT.temp_file("") as temp_ledger_file:
