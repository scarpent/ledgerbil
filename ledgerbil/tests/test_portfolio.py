...
def test_get_portfolio_report_compare_labels_account_only(mock_get_data):
    mock_get_data.return_value = portfolio_data
    args = portfolio.get_args(["--labels", "401k smactive", "--compare-accounts"])
    report = portfolio.get_portfolio_report(args)
    helper = OutputFileTester("test_portfolio_report_compare_labels_account_only")
    helper.save_out_file(report)
    helper.assert_out_equals_expected()
...
def test_account_matching_regex(mock_get_data):
    mock_get_data.return_value = portfolio_data
    matched_accounts, _, included_years = portfolio.get_matching_accounts("idx$")
    expected_included_years = {"2014", "2015", "2016", "2017", "2019"}
...
def test_account_matching_labels(mock_get_data):
    mock_get_data.return_value = portfolio_data
    matched_accounts, matched_labels, _ = portfolio.get_matching_accounts(
        ".*", "smactive"
    )
...
def test_account_matching_multiple_labels(mock_get_data):
    ...
    assert matched_accounts == [portfolio_data[BIG_CO], portfolio_data[BIG_NAME]]
...
def test_account_matching_multiple_labels_space_and_comma_stuff(mock_get_data):
    mock_get_data.return_value = portfolio_data
    matched_accounts, _, _ = portfolio.get_matching_accounts(
        ".*", "intl,  large  bonds"
    )
    expected = [portfolio_data[BIG_CO], portfolio_data[BONDS], portfolio_data[BIG_NAME]]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([[1.5, 1.5], 2], 50),
        ([[0.5, 0.5], 2], -50),
        ([[1, 1.25, 1.75, 0.75], 4], 13.175476395946738),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([[1, 1.25, 1.75, 0.75], 4], f"{13.18:{portfolio.COL_GAIN}.2f}"),
        (
            [[3, 3, 3, 3, 3, 3, 1, 1.25, 1.75, 0.75], 4],
...
def test_get_formatted_gain(test_input, expected):
    assert (
        Colorable.get_plain_string(portfolio.get_formatted_gain(*test_input))
        == expected
    )
...
def test_get_portfolio_data():
    ...
    expected = {"key": "value"}
    with mock.patch(
        __name__ + ".portfolio.open", mock.mock_open(read_data=jsondata)
    ) as m:
...
def test_get_account_history_one_year():
    account = {
        "account": "assets: 401k: bonds idx",
        "labels": [],
        "years": {"2016": {"price": 119.76, "shares": 3750.9, "contributions": 750}},
    }
...
def test_get_yearly_combined_accounts_single_account_missing_years():
    ...
    actual = portfolio.get_yearly_combined_accounts(
        accounts, {"2014", "2013", "2015", "2017", "2016"}
    )
    assert actual == expected
...
def test_get_yearly_combined_accounts_multiple_accounts_missing_years():
    ...
    actual = portfolio.get_yearly_combined_accounts(
        accounts, {"2014", "2013", "2015", "2017", "2016", "2018"}
    )
    assert actual == expected
...
def test_get_yearly_assert_gain_less_than_zero():
    ...
    totals = {2013: {"contributions": 10, "transfers": -20, "value": 30}}
...
def test_get_sorted_comparison_items(sort, first_row):
    items = [first_row, portfolio.Summary("x", 1, 1, 1, 1, 1, 1, 1, 1)]
...
def test_get_comparison_report_line():
    comparison_item = portfolio.Summary(
        "gargle", 2500, 600.0, 11, 4.02, 5.55, 7.04, 5.55, 3.6
    )
    line = portfolio.get_comparison_report_line(comparison_item, 50, True)
    expected = (
        "gargle            $ 2,500   50        $ 600  11     "
...
def test_get_comparison_report_line_no_value():
    comparison_item = portfolio.Summary(
        "gargle", 0, 600.0, 11, 4.02, 5.55, 7.04, 5.55, 3.6
    )
...
@pytest.mark.parametrize(
    "test_input, expected", [(["-H"], True), (["--history"], True), ([], False)]
)
...
@pytest.mark.parametrize(
    "test_input, expected", [(["-l"], True), (["--list"], True), ([], False)]
)
...
@pytest.mark.parametrize(
    "test_input, expected", [(["-c"], True), (["--compare"], True), ([], False)]
)
...
@pytest.mark.parametrize(
    "test_input, expected",
    [(["-C"], True), (["--compare-accounts"], True), ([], False)],
)
