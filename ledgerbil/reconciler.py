...
class Reconciler(cmd.Cmd):
    ...
    def mark_or_unmark(self, args, mark=True):
        ...
        if args[0].lower() == "all":
            args = []
            for key, thing in self.current_listing.items():
                if (mark and not thing.is_pending()) or (
                    not mark and thing.is_pending()
                ):
                    args.append(key)
    ...
    def list_transactions(self, args=None):
        ...
        for thing in self.open_transactions:
            if (
                thing.thing_date > self.ending_date
                and not show_all
            ...
            print(
                ...
                "previous balance: {previous_balance}".format(
                    previous_date=Colorable(
                        "white", util.get_date_string(self.previous_date)
                    ),
                    previous_balance=self.get_colored_amount(self.previous_balance),
                )
            )
            newline = ""
        else:
        ...
        print(
            ...
            "cleared: {cleared}".format(
                ...
                end_date=Colorable("cyan", util.get_date_string(self.ending_date)),
            )
        )
    ...
    def set_statement_info(self):
        ...
        while True:
            ...
            new_date = self.get_response(
                prompt="Ending Date (YYYY/MM/DD)", old_value=date_str
            )
        ...
        if self.ending_balance is None:
            ...
            old_ending_balance = util.get_amount_str(
                self.ending_balance, decimals=self.get_decimals()
            )

        while True:
            new_ending_balance = self.get_response(
                prompt="Ending Balance", old_value=old_ending_balance
            )
    ...
    def get_statement_info_from_cache(self):
        ...
        if key in cache:
            self.ending_date, self.ending_balance = self.get_date_and_balance(
                key, cache, "ending"
            )

            self.previous_date, self.previous_balance = self.get_date_and_balance(
                key, cache, "previous"
            )
    ...
    def get_date_and_balance(self, key, cache, prefix):
        the_date = util.get_date(
            cache[key].pop(f"{prefix}_date", util.get_date_string(date.today()))
        )
...
def reconciled_status():
    ...
    accounts = {
        account: recon
        for account, recon in accounts_all.items()
...
def get_expanded_account_name(account):
    ...
    for alias_regex, replacement in aliases.items():
        expanded_account = re.sub(alias_regex, replacement, expanded_account)
...
def print_reconciled_status_line(
    previous_date, previous_balance, ledger_balance, account
):
    amount_col = ">12"
    print(
